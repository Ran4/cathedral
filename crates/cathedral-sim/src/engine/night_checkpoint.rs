...
impl EngineNightDtoV1 {
    pub(crate) fn complete_decode(
        ...
        struct V2 {
            ...
            queued_incarnations: Vec<Option<u64>>,
        }
        ...
        let w: Wire = match crate::checkpoint::complete::owner_version(bytes)? {
            ...
            2 => {
                ...
                owner::check(v.version == 2, "unsupported Night extension")?;
                v.base
                    .night
                    .night
                    .install_continuation_v2(v.load_retry_pending, v.queued_incarnations)?;
                v.base
            }
            _ => {
                return Err(crate::checkpoint::CheckpointError::new(
                    OWNER,
                    "unsupported Night complete version",
                ));
            }
        };
        let d = Self {
...
impl Engine {
    pub(crate) fn complete_write_night<W: std::io::Write>(
        ...
        if self.night.has_continuation() {
            #[derive(Serialize)]
            struct V2<T: Serialize, Q: Serialize> {
                version: u16,
                base: T,
                load_retry_pending: bool,
                queued_incarnations: Q,
            }
            crate::checkpoint::complete::write_json(
                ...
                &V2 {
                    version: 2,
                    base: view,
                    load_retry_pending: self.night.load_retry_pending(),
                    queued_incarnations: self.night.queued_incarnations(),
                },
            )
        } else {
