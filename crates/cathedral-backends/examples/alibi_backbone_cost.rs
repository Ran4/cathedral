//! M2a2 private component probe. No full-save or synchronous-host budget claim.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for _ in 0..args.samples {
        ...
        let preflight = time(&mut phases.preflight_us, || {
            w.checkpoint_backbone_cost(initial())
        })?;
        let cost = *preflight.value();
        drop(preflight);
        let dto = time(&mut phases.export_us, || {
            w.export_backbone_checkpoint(initial())
        })?;
        ...
        let decoded = time(&mut phases.decode_validate_us, || {
            WorldBackboneDtoV1::decode(bytes.value(), input, &w.item_catalog, &w.command_ledger)
        })?;
        let candidate = time(&mut phases.candidate_validate_us, || {
            decoded.into_candidate(&w.item_catalog, &w.command_ledger)
        })?;
        assert_eq!(candidate.value().counts(), counts);
        let current = json!({"counts":counts,"cost":cost,"shared_reserved_peak_excluding_running_bytes":budget.retained_bytes()});
