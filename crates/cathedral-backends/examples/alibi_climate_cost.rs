//! M2a4 clock/weather component probe (forced storm with old-rate bell obligations). No full-save or synchronous-host budget claim.
...
use cathedral_sim::{
    ...
    RequestId, ShelterMap, SoundCatalog, TtsBackendKind, Vec3, WeatherKind, WorldClock,
    checkpoint::{CheckpointBudget, Cohort},
    engine::climate_checkpoint::{ClimateCheckpointContext, EngineClimateDtoV1},
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let boundary = LogicalTime::new(13.0).unwrap();
    let w = engine.world();
    let context = ClimateCheckpointContext::from_world(w, boundary);
    ...
    for _ in 0..args.samples {
        ...
        let preflight = time(&mut phases.preflight_us, || {
            engine.checkpoint_climate_cost(boundary, initial())
        })?;
        let cost = *preflight.value();
        drop(preflight);
        let dto = time(&mut phases.export_us, || {
            engine.export_climate_checkpoint(boundary, initial())
        })?;
        let counts = dto.value().counts(context);
        assert!(counts.bell_strokes > 0 && counts.weather_forced && !counts.weather_residue);
        ...
        let decoded = time(&mut phases.decode_validate_us, || {
            EngineClimateDtoV1::decode(bytes.value(), input, context)
        })?;
        ...
        let current = json!({"scenario":"forced_storm_with_old_rate_bells","counts":counts,"cost":cost,"shared_reserved_peak_excluding_running_bytes":budget.retained_bytes()});
