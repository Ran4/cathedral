Status: Reconciled 2026-09-22. M1–M2 and M3 through native retention retain their earlier acceptance records. Diagnostic sinks, admitted startup services, refusal-only controls, screenshot fixes, and partial M4/M5 foundations are committed; the corrected M6 current-authority slice passed independent review and 36 focused/compatibility tests. M3 actor-source capture, pinned Linux/glibc native directory accounting and the shared embedded shelter definition now have focused verification; other parsed definitions, allocator/platform modes, configuration and complete application accounting remain open. Complete M3 allocation/adoption/save-load and frame acceptance remain unfinished, so M4–M6 are not accepted milestones and M7–M19 have not started. M0 renderer/full-stress evidence remains pending.
