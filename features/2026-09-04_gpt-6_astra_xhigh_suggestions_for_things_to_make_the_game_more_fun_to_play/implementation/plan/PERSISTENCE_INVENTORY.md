...
| `dogs.rs` / `World.dogs` | Exact ordered dogs, all body/prose/path/rest/epoch fields | M2a8 S existing private authority; preserve signed rest and wrapping epoch, including arrival's empty path/positive speed. Exact immutable World/config nav bind independently. Rebuild bodies/animation handles separately |
| `World.ward_moods` | Settled nightly ward mood | M2a9 S exact independent values, including public edits; never invent or trim a replacement on load |
...
| `World.spoke_this_turn` | Same-turn seizure guard and scheduler seated-fact reheat | M2a2 S exact marker; full reply/pending agreement must bind it to the seated obligation and must never cut multi-action application in half |
...
| `Engine` | `last_clock_days`, `last_weather_days`, `last_weather_sample`; movement/round cadence and other elapsed owners | M2a4 S exact climate/clock cursors and sampled fields at the supplied boundary. M2a8 saves the shared human/dog movement_now anchor exactly; host residual assembly, Round cadence, player sound and other temporal owners remain explicitly pending with their own sentinel policies |
...
Conversation/CapturedAttention, pending provider/request authority, scheduler/Night, remaining Engine config/cadence, marks/animals and complete envelope adoption remain pending; existing law/custody/notices is covered by the M2a6 delta below. No constructor/reseed/partial production install exists. Existing allocation and host-frame limits remain unchanged, and naive simultaneous component composition is already over 1 GiB before this cut. M2b must establish actual phase lifetimes, Running/retiring admission and full temporal/accounting horizons.


## M2a6 existing law component delta — 2026-09-08

Notices.live/next_id and every notice's private served and settlement linkage are now strict S authority. Custody.held preserves every record, ordered unique holder, frozen Station and logical/calendar anchor. Committed officer names and notice links may outlive the live roster/notice board; active holder/InCharge officer bindings must resolve to the supplied saved backbone. No load-time expiry, release, lookup substitution or authored inmate seed is permitted. This preserves existing law, not future M11 hearing mechanics.

Engine.last_law_standing is S exact mandatory-nullable publication history, together with the component's immutable player_id copy. It may lag a public World mutation until the next poll; fixed geometry/fee/prose/order/numerical relations are validated without replacing historical names/anchors. Host initial republish remains a separate presentation action. Host strain/struggling_reported and pending threshold command/watermark are still mandatory full-host ownership; neither progress reset nor double escape is accepted. World.spoke_this_turn is already M2a2 S and also gates scheduler seated reheat, so the future reply/pending owner must validate those obligations together.

[Coverage](evidence/m2a6/OWNER_COVERAGE.md) and [admission](evidence/m2a6/ADMISSION.md) distinguish exact current registry context from frozen destinations, component headroom from full command/catch-up horizons, and per-owner candidates from whole-world adoption. Remaining Marks/catalog/switches/chalk cache, dogs, ward moods/Night, complete Engine configuration/cadence, conversation/attention/speech/scheduler and external pending work stay explicitly uncovered.


## M2a7 existing marks component delta — 2026-09-08

[Owner coverage](evidence/m2a7/OWNER_COVERAGE.md) now saves every private Marks field, all live mark records, World switches and the exact Engine last_chalk_standing plus initial marks config. Orphan household/place anchors remain saved until the ordinary gated sweep; historical about/author references and duplicate low-level kind/anchor pairs are not reconstructed. Spent daily work survives an empty scrubbed store. Calendar rewind may retain a later drawn date than the refreshed decay anchor. Scale/config IEEE bits preserve reachable NaN/infinity/nonpositive values for the unchanged ordinary fallback.

Exact supplied MarkCatalog resolved fields and ordered registry/shelter definitions bind borrowed World or unadopted backbone context. No default catalog substitution, parsing or seeding runs during candidate conversion. World.current_time is a checked backbone copy; world_revision remains M2a2 authority. Cached chalk handles/names/order/pen may lag a public mutation, and duplicate normal place names may yield duplicate handles; the next ordinary publisher determines a change exactly once.

Private World/Engine and ordinary Round continuation tests cover raw fields, historical/orphan references, immediate canonical equality after scrambled component install, decay/cleanup/scrub/daily redraw, allocator saturation, zero/saturated tally increments, source choice and publication/revision behavior. [Admission](evidence/m2a7/ADMISSION.md) preserves original raw charges and proves closed containers/context scratch under unchanged caps. No complete envelope, production hydration or host initial republish is declared. Dogs, independent ward moods/Night, remaining configuration/cadence, attention/conversation/scheduler/speech and pending external work remain uncovered. [Coordinator review](evidence/m2a7/coordinator/review.md) accepts M2a7 as a component, with 1,986 workspace passes and 3,600 audited release phase samples. Complete M2/M3 adoption remains pending.


## M2a8 existing animals and shared movement cadence delta — 2026-09-08

[Owner coverage](evidence/m2a8/OWNER_COVERAGE.md) saves ordered World.dogs and every Dog field: id, name, description, coat, build, base, leash_m, position_m, facing_yaw, speed, gait_phase, private path/rest_s/epoch. No DogView, authored seed row or rerouted drift substitutes for authority. Signed rest/public edits, planted turns, partial acceleration, arrival/stop and wrapping MAX epoch survive exact component restoration.

EngineAnimals owns dogs_published and the shared human/dog movement_now logical elapsed anchor, with the immutable player ID and config.nav binding. World.nav and config.nav bind separately after public World edits. The movement anchor is saved directly in the accepted elapsed coordinate system without lossy subtraction/re-addition; complete capture must bind human/dog/custody movement and host residuals to this single value. Nav=None retains an old anchor and unspent publication, and historical backward/debug boundaries preclude a universal residual/order rule at the component seam.

[Admission](evidence/m2a8/ADMISSION.md) retains raw padding/escape charges and a justified 1 MiB bounded borrowed-ID index allowance under the unchanged global caps. Context supports live World or an unadopted BackboneCandidate, without allocating registry/route indexes or warming navigation caches. Private continuation scrambles independent owners then proves immediate canonical equality and matching ordinary stepping/polls; independent public boundaries verify exact nav, paths/rest/epochs and raw refusal semantics.

Complete envelope/build/target/toolchain/DefaultHasher binding, all-consumer numeric/temporal horizons, shared Engine configuration equality, independent host initial republish, Running/Save/Load/retiring lifetimes, capture/hydration and pending external work remain mandatory. Ward moods/Night and remaining Engine configuration/cadence/attention/conversation/scheduler/speech owners remain uncovered. Final M2a8 verification passes 2,004 workspace tests, and coordinator release acceptance is recorded in the [M2a8 handoff](evidence/m2a8/README.md); this component is not a complete save.


## M2a9 existing Night component delta — 2026-09-08

[Owner coverage](evidence/m2a9/OWNER_COVERAGE.md) now saves every existing NightOffice field, exact independent World.ward_moods and original Engine.config.night_office. Ordered unadmitted/Busy-admitted duties, old day and incarnation, exact submitted prompt/request/root, held success or separate error kind/detail/duration, queue-time last_reflected, resolved historical bedtimes, office/ambient guards, both pacing anchors, seeded/config and totals are strict mandatory authority. An old flight may coexist with a newer same-subject stamp and duty; absent/replaced subjects and obsolete days remain until ordinary checks consume them.

NightCheckpointContext borrows either World or unadopted backbone and ledger, with the exact bitwise live clock binding. Clock and ledger stay their existing owners. A narrow traits adapter reconstructs exact Completion/CognitionError fields; no constructor/reseed/provider-failure/idle helper or public partial installation is used. Reachable positive-infinite historical pacing is explicit Never, and finite nonnegative At preserves signed zero. Supported local counter headroom accounts for the next ring's bounded new subjects plus the old queue and harvested flight; full temporal/rate/accounting horizons remain pending.

[Admission](evidence/m2a9/ADMISSION.md) preserves raw padding/escape charges and proves sequential borrowed ledger/queue scratch under a 4 MiB allowance, unchanged global caps. World moods use general admitted text limits, independently of the normal writer's 600-character cap. The authored/+2,000 probe has actual 520/2,520 actors and ordinary Busy, submitted and held obligations plus an applied ward mood; rare/max shapes remain separate tests.

This opaque existing-owner component is independently reviewed and accepted, with 2,025 passing workspace tests and 3,600 audited release phase samples; see [coordinator review](evidence/m2a9/coordinator/review.md). Complete M2c must restore completed terminal values once, replace unfinished executions with exactly one load-specific retry inside the saved valid window, preserve original inputs and reject old-generation callbacks. Complete M2a envelope, M2b capture/hydration, remaining Engine/social/scheduling/speech/host owners, initial publication and Running/Save/Load/retiring lifetime strategy remain mandatory. The prior naive backbone+Round Save+Load total already exceeds 1 GiB excluding Running; no caps increase.


## M2a10 existing social component delta — 2026-09-08

[Owner coverage](evidence/m2a10/OWNER_COVERAGE.md) saves every Conversation, WarmExchanges and Novelty field, plus Engine's original idle_mode, stage, idle_requires_news, idle_curiosity and immutable player identity. Independent incumbent/invitation/focus, partial dwell, capped prior witnesses, saturating monotonic tokens, canonical historical warm pairs and exact opaque context/visit salts survive. No expiry, current-body filtering, target-is-witness gate, context rehash or scheduler reconstruction occurs during decode. Old attention comments that prohibited saving these owners are corrected without ordinary behavior changes.

Engine context binds the exact player against borrowed World or an unadopted backbone. Social anchors retain logical coordinates directly, including signed zero and finite historical future/backwards samples within the supported format domain; supplied boundary agreement is bitwise. Full accepted-time and all-consumer chronology remains the envelope's responsibility. Raw Stage radius/Curiosity scale bits preserve NaN payloads, infinities and negative zero for ordinary fallback, and checked fixed-width max_actors retains zero and usize::MAX.

[Admission](evidence/m2a10/ADMISSION.md) retains complete raw byte/escape/padding charges, direct unique maps/sets and a 64 KiB fixed working allowance under unchanged caps. Explicit format limits are 128 witnesses, 25,000 warm pairs independently of roster and 25,000 Novelty rows; malformed overcount/key allocation remains aggregate-bounded before typed parsing. Private tests scramble independently prepared owners, demand immediate canonical equality and then continue ordinary speech, focus, news, expiry and Engine polling. New social fixtures preserve prior fixture bytes. [Coordinator acceptance](evidence/m2a10/coordinator/review.md) verifies 2,045 workspace passes and 3,600 release phase samples, with complete source/log/admission audits.

CapturedAttention is not added as a standalone DTO: interrupted unsent microphone/STT captures/jobs are discarded by the later draft policy, with counters and already committed suffixes retained. Scheduler/Floor/SpeechRouter, remaining Engine config/cadence, external generation fencing and M2c adoption stay pending. This component does not complete M2a envelope, M2b capture/hydration, host initial publication or Running/Save/Load/retiring lifetime composition. Exact content/build/target/toolchain/DefaultHasher agreement remains mandatory before a complete save can be adopted. Naive backbone+Round Save+Load alone already exceeds 1 GiB; no caps increase.

## M2a11 existing scheduler component delta — 2026-09-08

[Owner coverage](evidence/m2a11/OWNER_COVERAGE.md) now records exact weighted order/cursor, both FIFO queues, normalized delays and historical pacing, failure counter, running/submitted notification, retry map and full flight/held terminal authority. Flight preserves actor/incarnation/request/root/lane, drained inbox, presented history and original prompt; completion preserves exact success or error kind/detail and duration. Original Engine turn-delay/backoff floats are independent strict IEEE bit records and player identity binds the borrowed body. Implementation verification and coordinator acceptance are tracked in the [handoff](evidence/m2a11/README.md).

Valid historical IDs and stale incarnations remain until ordinary consumers retire them. Queues are disjoint and unique, but a flying actor can be queued again for later input. Root validation borrows live or unadopted saved ledger authority with TURN_PRODUCER and allows known committed receipts for replay suppression. No constructor, prompt rendering, generic failure, ordinary poll, temporary World/ledger candidate, public scheduler Clone or partial production installation occurs during decoding.

[Admission](evidence/m2a11/ADMISSION.md) proves bounded raw parsing/copying and sequential borrowed ledger/lane/root scratch with a 4 MiB working allowance, unchanged E/J/depth/shared caps. Supported limits are 100,000 weighted slots, 25,000 per lane, 256 retries, 64 rows per input buffer, 65,536 bytes per input string/prompt and 400,000 bytes per held success. Scheduler floor deferral precedes the provider's 100,000-scalar predicate; decode must retain an oversized held answer for ordinary failure, rather than treating it as a prevalidated Night success. Next failure increment is safe through the signed i32 exponent boundary; all-future horizon safety remains pending.

Complete Engine capture must reconcile the scheduler's submission notification with the consumed Novelty stamp and seated knowledge/receipt owners. Full M2c must retain every unfinished obligation, including empty-inbox idle work, and resubmit exactly one load-specific retry of original resolved input with generation fencing. Derived provider output-token budget is not separately stored in current flight and remains an explicit resolved-input receipt/content-binding obligation. Held terminal values need no new provider call. Floor/SpeechRouter draft transformation, remaining Engine configuration/cadence, complete manifests, host initial publication and actual Running/Save/Load/retiring lifetimes remain mandatory; this is not a complete supported save or host-frame acceptance.

### M2a12 floor/Engine continuity owner delta — 2026-09-08

The exact admitted `FloorDtoV1` and `EngineContinuityDtoV1` cover the existing Floor and remaining Engine cadence/publication/configuration authority. [Coverage](evidence/m2a12/OWNER_COVERAGE.md) and [admission](evidence/m2a12/ADMISSION.md) specify every covered field, limits, current-consumer gates and later owner agreements.

| Covered field family | Checkpoint disposition |
|---|---|
| `ConversationFloor.awaiting`; `AwaitedSpeech.event_id/deadline/blocks_player_reaction` | S exact ordered <=32 unique arbitrary SpeechEventId strings, <=65,536 UTF-8 bytes each; finite nonnegative/+infinite pacing. No roster/prefix rule, pruning, sorting or future-anchor normalization. |
| `ConversationFloor.foreground_floor_until/background_floor_until/player_hold_until` | S exact existing authority for this component. Full M2c later transforms old audio acknowledgements and transient player holds together with saved speech obligations/owed readable presentation; exact component capture does not claim restored audio liveness. |
| `Engine.last_snapshot_revision/lamp_revision_sent/ready_emitted/startup_diagnostics` | S exact i64/u64 publication cursors, ready flag and ordered diagnostics (<=25,000 rows, <=65,536 bytes each). No forced refresh, constructor or extra poll. M3 initial publication remains a separate explicit presentation operation. |
| `Engine.last_player_sound_at/next_round_tick_at` | S exact cooldown and Round cadence anchors; explicit negative-infinity Never for never sounded, supported future pacing without rebasing historical anchors. |
| `Engine.tts_selected`; remaining `EngineConfig.fake_mode/sounds_enabled/view_cone_degrees/sound_cooldown_seconds/tts_selected/tts_startup_message/stt_stream_grace_seconds` | S independent actual voice selection and current stored configuration. Preserve raw IEEE config bits; constructor already clamps cooldown/floors grace, which must not be repeated. Nullable startup text remains mandatory and byte-limited. |
| `EngineConfig.player_id` | R immutable exact binding to the supplied player and borrowed live World or unadopted BackboneCandidate membership, without a presence/control predicate. |
