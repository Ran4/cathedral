...
# dependencies = []
# ///
"""Measure M2a6 existing law authority on authored and +2,000 resident worlds.
...
def main():
    ...
    binary = ROOT / "target/release/examples/alibi_law_cost"
    before = sources()
    ...
    identity = {
        ...
        "measurement": "Sequential release component API calls; six raw phase timings. "
            ...
            "The diagnostic retains live law rungs, served notices, active and authored "
            "custody, expired notice links and historical officers before ordinary "
            "cache publication. Expanded/admission charges are "
    ...
    for repeat in range(repeats):
        ...
        for mode in modes:
            ...
            if (data["scenario"] != "law-obligations-v1"
                    or any(counts[key] != value for key, value in expected.items())):
                raise RuntimeError("expected active rungs, service, custody and publication cache")
            if witnesses != {"historical_notice_links": 1, "historical_officers": 1}:
                raise RuntimeError("expected retained expired notice and departed officer history")
            cost = data["cost"]
            ...
            if cost["validation_working_bytes"] != 4096 * 1024:
                raise RuntimeError("law validation working charge differs from the reviewed bound")
            if cost["peak_bytes"] != (4096 + 4 * cost["expanded_upper_bytes"]
                    + 3 * cost["encoded_bytes"] + cost["validation_working_bytes"]):
                raise RuntimeError("Law component peak accounting is inconsistent")
