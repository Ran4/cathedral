...
# dependencies = []
# ///
"""Measure M2a9 existing Night authority on authored and +2,000 resident worlds.
...
def main():
    ...
    binary = ROOT / "target/release/examples/alibi_night_cost"
    before = sources()
    ...
    identity = {
        ...
        "measurement": "Sequential release component API calls; six raw phase timings. "
            ...
            "The diagnostic retains ordinary queued duties, a submitted exact prompt, a capacity-held reply, "
            "settled ward mood and observed prior Busy retry/application. "
    ...
    for repeat in range(repeats):
        ...
        for mode in modes:
            ...
            expected = {
                "characters": 520 + extra, "bedtimes": 30, "queued": 6,
            ...
            if data["scenario"] != "night-obligations-v1" or counts != expected:
                raise RuntimeError("expected queued wards, one exact held reply and a settled mood")
            expected_witnesses = {
            ...
            if witnesses != expected_witnesses:
                raise RuntimeError("ordinary Busy, submit, apply and receipt deferral witnesses differ")
            cost = data["cost"]
            ...
            if cost["validation_working_bytes"] != 4096 * 1024:
                raise RuntimeError("Night validation working charge differs from the reviewed bound")
            if cost["peak_bytes"] != (4096 + 4 * cost["expanded_upper_bytes"]
                    + 3 * cost["encoded_bytes"] + cost["validation_working_bytes"]):
                raise RuntimeError("Night component peak accounting is inconsistent")
