...
final_names=['checkpoint_bounded_final','public_bounded_final','workspace_serial']
results={}
...
for mode in ['authored','populated']:
    p=base/'development'/(mode+'_bounded_smoke.json');j=json.loads(p.read_text());assert all(len(j[k])==j['samples'] for k in ['preflight_us','export_us','encode_us','decode_validate_us','candidate_validate_us','drop_us'])
    assert j['witnesses']['coarse_discard_diagnostics']==0 and 0<j['witnesses']['maximum_poll_step_seconds']<=0.05+1e-12 and j['witnesses']['poll_count']>=100
    assert j['counts']['characters']==(520 if mode=='authored' else 2520)
    assert len(j['witnesses']['submitted_prompts'])==3 and j['counts']['held_success'] and j['counts']['retry_work']==1
