...
for name in ['source_audit','format_audit','log_archive_audit','stdlib_allocation_audit','release_archive_audit','scheduler_smoke_audit','scheduler_performance_audit','auditor_regressions','tail_latency_audit','evidence_link_audit']:
...
script_hashes = {}
for p in sorted(Path('/tmp').glob('alibi-m2a11-*.py')):
    target = root/e/'coordinator'/p.name.removeprefix('alibi-m2a11-')
...
paths += [str(p) for p in [feature/'README.md',plan/'README.md',plan/'M2_simulation_checkpoints.md',plan/'PERSISTENCE_INVENTORY.md',here/'audit_m2_component_probes.py',here/'run_m2_scheduler_probes.py',here/'component_input_sources.py',e]]
