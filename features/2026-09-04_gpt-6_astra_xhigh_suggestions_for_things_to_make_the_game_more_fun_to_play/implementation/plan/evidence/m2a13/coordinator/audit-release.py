...
assert build['binary_sha256']['alibi_speech_cost'] == binary_hash
...
for directory in ['smoke','performance']:
    ...
    original = Path(f'/tmp/alibi-m2a13-release-{directory}-runner.log')
...
for mode in ['authored','populated']:
    debug = load(e/f'development/{mode}_final_smoke.json')
    measured = summary[mode]['metadata']
    for key in ['counts','witnesses','boundary_speech','cost','shared_reserved_peak_excluding_running_bytes']:
