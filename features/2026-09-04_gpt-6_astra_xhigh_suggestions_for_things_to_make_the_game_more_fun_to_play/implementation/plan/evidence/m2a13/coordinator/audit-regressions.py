...
cases = {
    ...
    'changed_boundary_draft':'assert sha(json.dumps(data["boundary_speech"], sort_keys=True,',
    'missing_runtime_defaults':'assert component_sources() == identity["source_sha256"]',
}
for name,expected_gate in cases.items():
    folder = Path(tempfile.mkdtemp(prefix='alibi-m2a13-auditor-'+name+'-'))/'data'
    ...
    if name == 'percentile':
        ...
        elif name == 'lost_recording':
            data['counts']['accepted_recordings'] = 0
        elif name == 'coarse_step':
            data['witnesses']['maximum_poll_step_seconds'] = .35
        elif name == 'changed_boundary_draft':
            data['boundary_speech']['state']['streams'][0]['available_text'] += ' changed'
        else:
    ...
    if name == 'missing_runtime_defaults':
        command += ['--check-current',str(binary)]
...
path = here/'m2a13/coordinator/auditor_regressions.json'
