...
assert git('rev-parse', 'HEAD').decode().strip() == 'c035a430da9d23132bcf9f8138e27eedca3401bf'
...
allowed_files = {
    ...
    'crates/cathedral-sim/src/speech_router.rs',
    'crates/cathedral-sim/src/receipts.rs',
    'crates/cathedral-sim/src/receipts/checkpoint.rs',
    'crates/cathedral-sim/src/engine/speech_checkpoint.rs',
    'crates/cathedral-sim/tests/checkpoint_speech_boundary.rs',
    'crates/cathedral-backends/examples/alibi_speech_cost.rs',
    'crates/cathedral-sim/tests/fixtures/checkpoint_engine_speech_empty_v1.json',
    'crates/cathedral-sim/tests/fixtures/checkpoint_engine_speech_onset_v1.json',
    'crates/cathedral-sim/tests/fixtures/checkpoint_speech_empty_v1.json',
    'crates/cathedral-sim/tests/fixtures/checkpoint_speech_pending_v1.json',
}
allowed_prefixes = (
    'crates/cathedral-sim/src/speech_router/checkpoint',
    'crates/cathedral-sim/src/engine/speech_checkpoint/',
    'crates/cathedral-sim/src/receipts/checkpoint/speech',
)
assert all(p in allowed_files or p.startswith(allowed_prefixes) for p in changed), sorted(changed)
comparisons = []
additions = {
    'crates/cathedral-sim/src/engine.rs': 'pub mod speech_checkpoint;\n',
    'crates/cathedral-sim/src/speech_router.rs': 'pub mod checkpoint;\n\n',
    'crates/cathedral-sim/src/receipts.rs': 'pub(crate) use checkpoint::{CheckpointReceiptRef, validate_speech_receipt};\n',
...
for name, addition in additions.items():
    ...
    comparisons.append({'path': name, 'baseline_sha256': sha(before), 'current_sha256': sha(after),
                        'removed_wiring_bytes': addition, 'remaining_bytes_identical': True})
fixtures = {}
