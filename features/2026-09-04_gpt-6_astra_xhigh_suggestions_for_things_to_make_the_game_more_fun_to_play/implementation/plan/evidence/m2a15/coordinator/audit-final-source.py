...
assert re.fullmatch(r'[a-z0-9-]+', args.command)
assert re.fullmatch(r'[a-z0-9-]+', args.output_prefix)
frozen = frozen_sources()
start_path = LEG / f'commands/{args.command}.start.json'
start = json.loads(start_path.read_bytes())
result_path = LEG / f'commands/{args.command}.result.json'
...
assert gzip.decompress((LEG / f'commands/{args.command}.log.gz').read_bytes()) == raw
...
assert len(groups) == 46 and totals == [args.passed, 0, args.ignored], (len(groups), totals)
public = list(re.finditer(rb'^test host_checkpoint::tests_public::\S+ \.\.\. ', raw, re.M))
assert len(public) == 11
for match in public:
    following = raw[match.end():]
    next_test = re.search(rb'^test \S+ \.\.\. ', following, re.M)
    segment = following[:next_test.start()] if next_test else following
    assert re.search(rb'^ok$', segment, re.M), 'public test did not complete successfully'
...
original = Path(f'/tmp/alibi-m2a15-{args.output_prefix}-format.log')
assert not original.exists()
with original.open('wb') as stream:
    completed = subprocess.run(command, cwd=ROOT, stdout=stream, stderr=subprocess.STDOUT)
format_log = archive(original, OUT / f'{args.output_prefix}-format.log.gz')
...
destination = OUT / f'{args.output_prefix}.json'
