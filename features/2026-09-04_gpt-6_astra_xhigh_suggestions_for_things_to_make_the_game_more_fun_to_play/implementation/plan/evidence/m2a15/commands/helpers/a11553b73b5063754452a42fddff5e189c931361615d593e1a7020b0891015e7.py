...
identity = {"started_utc": datetime.now(timezone.utc).isoformat(),
            ...
            "environment": {k: os.environ.get(k) for k in ("PATH", "CARGO_HOME", "RUSTC", "RUSTDOC", "RUSTFLAGS", "CARGO_TARGET_DIR", "CATHEDRAL_HEADLESS", "CATHEDRAL_FAKE_BACKEND")},
            "raw_log": str(raw)}
...
with (out / (label + ".log.gz")).open("wb") as dest:
    with gzip.GzipFile(filename="", mode="wb", fileobj=dest, mtime=0) as stream:
        stream.write(data)
(out / (label + ".result.json")).write_text(json.dumps({"exit_code": result.returncode, "raw_sha256": hashlib.sha256(data).hexdigest(), "completed_utc": datetime.now(timezone.utc).isoformat()}, indent=2) + "\n")
