...
# dependencies = []
# ///
"""Measure M2a8 existing animal authority on authored and +2,000 resident worlds.
...
def main():
    ...
    binary = ROOT / "target/release/examples/alibi_animals_cost"
    before = sources()
    ...
    identity = {
        ...
        "measurement": "Sequential release component API calls; six raw phase timings. "
            ...
            "The diagnostic retains the ordinary pack with concurrent resting, moving and turning dogs, "
            "the exact shared movement slice and prior publication history. "
    ...
    for repeat in range(repeats):
        ...
        for mode in modes:
            ...
            expected = {
                "characters": 520 + extra, "dogs": 10, "decided": 2,
                "path_dogs": 2, "waypoints": 16, "resting": 8, "moving": 1,
                "turning": 1, "stop_pending": 0,
                "dogs_published": True, "engine_nav": True,
            }
            if (data["scenario"] != "animals-obligations-v1" or counts != expected):
                raise RuntimeError("expected the ordinary resting/moving/turning pack and publication state")
            if (any(witnesses[key] is not True for key in (
                    "initial_all_resting", "turning_observed", "acceleration_observed"))
                    or witnesses["initial_resting_publications"] != 1
                    or witnesses["quiet_publications"] != 0
                    or witnesses["movement_publications"] != 27
                    or witnesses["boundary_seconds"] != 6.061000000000001
                    or witnesses["movement_now_seconds"] != 6.0499999999999865
                    or witnesses["cadence_residual_seconds"] != (
                        witnesses["boundary_seconds"] - witnesses["movement_now_seconds"])
                    or not 0 < witnesses["cadence_residual_seconds"] < 0.05):
                raise RuntimeError("expected once-only resting publication and ordinary motion/cadence witnesses")
            cost = data["cost"]
            ...
            if cost["validation_working_bytes"] != 1024 * 1024:
                raise RuntimeError("animals validation working charge differs from the reviewed bound")
            if cost["peak_bytes"] != (4096 + 4 * cost["expanded_upper_bytes"]
                    + 3 * cost["encoded_bytes"] + cost["validation_working_bytes"]):
                raise RuntimeError("Animals component peak accounting is inconsistent")
