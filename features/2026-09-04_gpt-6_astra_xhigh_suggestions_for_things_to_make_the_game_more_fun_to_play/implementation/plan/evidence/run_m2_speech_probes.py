...
# dependencies = []
# ///
"""Measure M2a13 interrupted speech inputs on authored and +2,000 resident worlds.
...
EXPECTED_COUNTS = {'authored': {'accepted_recordings': 3,
              ...
              'terminal_receipts': 0,
              'unique_roots': 2},
 'populated': {'accepted_recordings': 3,
               'available_text_bytes': 85,
               'available_texts': 1,
               'basename_bytes': 84,
               'batch_pending': 2,
               ...
               'terminal_receipts': 0,
               'unique_roots': 2}}
EXPECTED_WITNESS_HASHES = {'authored': '7050dbf38118bd6a20b1e3defd0294149b6c713f97c0291c32e0391ed814c152',
 'populated': '24fb8c6a244d97f83920a9daaccd66ea31b6d5b7aba36afcff8c8ea6adf87dca'}
EXPECTED_BOUNDARY_HASHES = {'authored': 'c2ddace801ea300da6ffd66c61e85713d12fdc33f7e5c5934625de0372617240',
 'populated': '3db32db9acdf1d93e81e37b3b48f29a530b9941067949c056907835bac9cb3c4'}
...
def main():
    ...
    binary = ROOT / "target/release/examples/alibi_speech_cost"
    before = sources()
    ...
    identity = {
        ...
        "measurement": "Sequential release interrupted speech component calls; six raw phases. "
    ...
    for repeat in range(repeats):
        ...
        for mode in modes:
            ...
            if data["scenario"] != "speech-ordinary-interrupted-inputs-v1" or counts != EXPECTED_COUNTS[mode]:
            ...
            if cost["validation_working_bytes"] != 4096 * 1024:
                raise RuntimeError("Speech validation working charge differs from the reviewed bound")
            if cost["peak_bytes"] != (4096 + 4 * cost["expanded_upper_bytes"]
                    + 3 * cost["encoded_bytes"] + cost["validation_working_bytes"]):
                raise RuntimeError("Speech component peak accounting is inconsistent")
