...
# dependencies = []
# ///
"""Measure M2a7 existing marks authority on authored and +2,000 resident worlds.
...
def main():
    ...
    binary = ROOT / "target/release/examples/alibi_marks_cost"
    before = sources()
    ...
    identity = {
        ...
        "measurement": "Sequential release component API calls; six raw phase timings. "
            ...
            "The diagnostic retains active marks, historical links, weathering and the "
            "spent daily ward beat before ordinary chalk cache publication. "
    ...
    for repeat in range(repeats):
        ...
        for mode in modes:
            ...
            expected = {
                ...
                "cached_anchors": 3 if extra else 1,
                "cached_kinds": 3 if extra else 1,
            }
            if (data["scenario"] != "marks-obligations-v1"
                    or any(counts[key] != value for key, value in expected.items())):
                raise RuntimeError("expected three mark kinds, historical authors, spent clocks and cache")
            if (any(witnesses[key] is not True for key in (
                    "scrubbed", "same_day_suppressed", "sweep_changed", "cache_deduped"))
                    or witnesses["chalk_publications"] != 1
                    or not 0 < witnesses["cross_strength"] < 1
                    or witnesses["revision_after_setup"] != (5111 if extra else 1111)):
                raise RuntimeError("expected real weathering, spent scrub day and once-only cache publication")
            cost = data["cost"]
            ...
            if cost["validation_working_bytes"] != 4096 * 1024:
                raise RuntimeError("marks validation working charge differs from the reviewed bound")
            if cost["peak_bytes"] != (4096 + 4 * cost["expanded_upper_bytes"]
                    + 3 * cost["encoded_bytes"] + cost["validation_working_bytes"]):
                raise RuntimeError("Marks component peak accounting is inconsistent")
