...
# dependencies = []
# ///
"""Measure M2a10 existing social authority on authored and +2,000 resident worlds.
...
def main():
    ...
    identity = {
        ...
        "measurement": "Sequential release component API calls; six raw phase timings. "
            "Decode and candidate validation include social and player binding gates. "
            "Ordinary typed speech, a scripted completion and a later gaze sample "
    ...
    for repeat in range(repeats):
        ...
        for mode in modes:
            ...
            expected = {
                "characters": 520 + extra, "engaged": True, "reciprocal": True,
                "focus": True, "invitation": True, "next_utterance": 1,
                "latest_applied_utterance": 1, "warm_pairs": 1,
                "novelty_told": 1, "novelty_memories": 6 if extra else 2,
                "witnesses": 6 if extra else 3,
            }
            if data["scenario"] != "social-continuity-v1" or counts != expected:
                raise RuntimeError("expected reciprocal focus, warm exchange and submission-time novelty")
            if witnesses["partner_retained"] is not True:
                raise RuntimeError("ordinary reciprocal partner was lost")
            # Exact input/output witnesses reviewed in the functional debug smokes.
            # Canonical hash includes every speech event, recipient, position,
            # actor, submitted prompt length, and the boundary.
            expected_witness_hashes = {
                "authored": "1a2e46149cb77aa29b8f4fabfdad4535a2bd28a9db6a59b3321dad76c29426b0",
                "populated": "d08eeb93362977d208c95e23009e8ea78b5cf821c4093dc4d11968b5e538ae14",
            }
            ...
            if witness_hash != expected_witness_hashes[mode]:
                raise RuntimeError("ordinary speech, submission or partial-focus witnesses differ")
            cost = data["cost"]
            ...
            if cost["validation_working_bytes"] != 64 * 1024:
                raise RuntimeError("Social validation working charge differs from the reviewed bound")
            if cost["peak_bytes"] != (4096 + 4 * cost["expanded_upper_bytes"]
                    + 3 * cost["encoded_bytes"] + cost["validation_working_bytes"]):
                raise RuntimeError("Social component peak accounting is inconsistent")
