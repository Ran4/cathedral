...
if any(x['name']==label for x in json.loads((Path('features/2026-09-04_gpt-6_astra_xhigh_suggestions_for_things_to_make_the_game_more_fun_to_play/implementation/plan/evidence/m2a14')/'commands.json').read_text())) if (Path('features/2026-09-04_gpt-6_astra_xhigh_suggestions_for_things_to_make_the_game_more_fun_to_play/implementation/plan/evidence/m2a14')/'commands.json').exists() else False: raise SystemExit('duplicate command label')
base=Path('features/2026-09-04_gpt-6_astra_xhigh_suggestions_for_things_to_make_the_game_more_fun_to_play/implementation/plan/evidence/m2a14')
log=Path('/tmp/alibi-m2a14-'+label+'.log')
config={'PATH':'/usr/bin:/bin:/home/ran/.local/bin:/home/ran/.cargo/bin','CARGO_HOME':'/tmp/alibi-m1b-cargo','RUSTC':'/home/ran/.cargo/bin/rustc','RUSTDOC':'/home/ran/.cargo/bin/rustdoc','CATHEDRAL_HEADLESS':'1','CATHEDRAL_FAKE_BACKEND':'1'}
...
manifest=sources()
helper_hash=hashlib.sha256((base.parent/'component_input_sources.py').read_bytes()).hexdigest()
