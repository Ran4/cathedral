...
original = Path('/tmp/alibi-m2a14-release-build-original.log')
timing = Path('/tmp/alibi-m2a14-release-build-original.time')
assert not original.exists() and not timing.exists()
assert not (out / 'release_build.json').exists()
cmd = ['/home/ran/.cargo/bin/cargo', 'build', '--offline', '--release', '-p', 'cathedral-backends', '--example', 'alibi_scheduler_cost', '--example', 'alibi_night_cost', '-j', '1']
...
report = {'command':cmd, 'environment_overrides':overrides, 'build_variable_whitelist':build_vars, 'started_utc':started, 'elapsed_seconds':elapsed, 'exit_code':result.returncode, 'artifacts':artifacts, 'source_scope':SOURCE_SCOPE, 'source_manifest_sha256':sha(e / 'source_hashes.json'), 'source_helper_sha256':helper_before, 'build_wrapper_sha256':wrapper_before}
...
if result.returncode == 0:
    ...
    report['preserved_references'] = {}
    for owner in ('scheduler', 'night'):
        name = 'alibi_' + owner + '_cost'
        binary = root / 'target/release/examples' / name
        reference = Path('/tmp/alibi-m2a14-' + owner + '-reference-binary')
        assert not reference.exists()
        shutil.copy2(binary, reference)
        report['binary_sha256'][name] = sha(binary)
        report['preserved_references'][name] = {'path':str(reference), 'sha256':sha(reference)}
...
assert sha(Path(__file__)) == wrapper_before
