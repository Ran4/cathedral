...
for key in ['final_focused', 'final_public', 'workspace']:
...
for name in ['probes_build', 'verify_smokes', 'rustc_version'] + [
        f'{owner}_{mode}_{kind}' for owner in ['scheduler', 'night']
        for mode in ['authored', 'populated'] for kind in ['inputs', 'default']]:
    record = next(r for r in records if r['name'] == name)
    assert record['exit_code'] == 0 and name in frozen_commands
workspace = verification['workspace']['command_name']
assert totals[workspace]['passed'] >= 2113 and totals[workspace]['ignored'] >= 33
