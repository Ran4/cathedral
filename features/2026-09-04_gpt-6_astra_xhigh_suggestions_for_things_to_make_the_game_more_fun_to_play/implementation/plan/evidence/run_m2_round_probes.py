...
# dependencies = []
# ///
"""Measure M2a3 private Round authority on authored and +2,000 resident worlds.
...
def main():
    ...
    binary = ROOT / "target/release/examples/alibi_round_cost"
    before = sources()
    ...
    for repeat in range(repeats):
        ...
        for mode in modes:
            ...
            if cost["definition_working_bytes"] != 4 * 1024 * 1024:
                raise RuntimeError("trusted-definition working charge differs from the reviewed bound")
            if cost["peak_bytes"] != (4096 + 4 * cost["expanded_upper_bytes"]
                    + 3 * cost["encoded_bytes"] + cost["definition_working_bytes"]):
                raise RuntimeError("Round component peak accounting is inconsistent")
            retained = data["shared_reserved_peak_excluding_running_bytes"]
            if not cost["peak_bytes"] <= retained <= 1024 * 1024 * 1024:
