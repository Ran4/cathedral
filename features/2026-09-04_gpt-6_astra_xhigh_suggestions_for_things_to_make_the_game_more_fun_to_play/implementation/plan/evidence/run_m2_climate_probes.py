...
# dependencies = []
# ///
"""Measure M2a4 private clock/weather authority on authored and +2,000 resident worlds.
...
def main():
    ...
    binary = ROOT / "target/release/examples/alibi_climate_cost"
    before = sources()
    ...
    for repeat in range(repeats):
        ...
        for mode in modes:
            ...
            if (data["scenario"] != "forced_storm_with_old_rate_bells"
                    or data["counts"]["bell_strokes"] != 3
                    or data["counts"]["weather_forced"] is not True
                    or data["counts"]["weather_residue"] is not False):
                raise RuntimeError("expected an active forced storm and three owed bell strokes")
            cost = data["cost"]
            ...
            if cost["validation_working_bytes"] != 64 * 1024:
                raise RuntimeError("climate validation working charge differs from the reviewed bound")
            if cost["peak_bytes"] != (4096 + 4 * cost["expanded_upper_bytes"]
                    + 3 * cost["encoded_bytes"] + cost["validation_working_bytes"]):
                raise RuntimeError("Climate component peak accounting is inconsistent")
            retained = data["shared_reserved_peak_excluding_running_bytes"]
            if not cost["peak_bytes"] <= retained <= 10264 * 1024:
