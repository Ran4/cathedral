...
cases = {
    ...
    'working_charge':'assert validation_working_bytes == 0',
    'lost_await':'assert counts == {',
    ...
    'changed_boundary_pacing':'assert sha(json.dumps(data["boundary_continuity"], sort_keys=True,',
    'missing_runtime_defaults':'assert component_sources() == identity["source_sha256"]',
}
for name,expected_gate in cases.items():
    folder = Path(tempfile.mkdtemp(prefix='alibi-m2a12-auditor-'+name+'-'))/'data'
    ...
    if name == 'percentile':
        ...
        elif name == 'lost_await':
            data['counts']['floor']['awaiting'] = 0
        elif name == 'coarse_step':
            data['witnesses']['maximum_poll_step_seconds'] = .35
        elif name == 'changed_boundary_pacing':
            data['boundary_continuity']['floor']['foreground_floor_until']['at'] += .01
        else:
...
path = here/'m2a12/coordinator/auditor_regressions.json'
