...
assert build['binary_sha256']['alibi_continuity_cost'] == binary_hash
...
for directory in ['smoke','performance']:
    ...
    original = Path(f'/tmp/alibi-m2a12-release-{directory}-runner.log')
...
for mode in ['authored','populated']:
    debug = load(e/f'development/{mode}_frozen_smoke.json')
    measured = summary[mode]['metadata']
    for key in ['counts','witnesses','boundary_continuity','cost','shared_reserved_peak_excluding_running_bytes']:
