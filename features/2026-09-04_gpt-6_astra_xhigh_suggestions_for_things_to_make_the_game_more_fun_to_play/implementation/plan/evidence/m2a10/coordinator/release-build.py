...
original = Path('/tmp/alibi-m2a10-release-build-original.log')
timing = Path('/tmp/alibi-m2a10-release-build-original.time')
assert not original.exists() and not timing.exists()
assert not (out / 'release_build.json').exists()
cmd = ['/home/ran/.cargo/bin/cargo', 'build', '--offline', '--release', '-p', 'cathedral-backends', '--example', 'alibi_social_cost', '-j', '1']
...
if result.returncode == 0:
    ...
    reference = Path('/tmp/alibi-m2a10-social-reference-binary')
    assert not reference.exists()
    shutil.copy2(binary, reference)
    report['binary_sha256'] = {'alibi_social_cost':sha(binary)}
