...
for name, expected_gate in cases.items():
    ...
    if name == 'percentile':
        ...
        if name == 'working_charge':
            ...
            data['witnesses']['partner_retained'] = False
    ...
    checks.append({'case':name,'command':command,'exit_code':result.returncode,'expected_failing_gate':expected_gate,'success_report_absent':not report.exists(),'output':output})
value = {'checked_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'auditor_sha256':sha(audit.read_bytes()),'measurement_summary_sha256':sha((source/'SUMMARY.json').read_bytes()),'result':'passed','checks':checks}
path = here / 'm2a10/coordinator/auditor_regressions.json'
