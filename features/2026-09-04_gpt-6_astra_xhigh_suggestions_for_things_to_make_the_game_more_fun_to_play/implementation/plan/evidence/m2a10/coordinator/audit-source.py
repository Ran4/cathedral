...
assert git('rev-parse','HEAD').decode().strip() == 'fd2f3d491a313881a6e71985ef6e6f486849903a'
...
ordinary = {
    'crates/cathedral-sim/src/engine.rs':'pub mod social_checkpoint;\n',
    'crates/cathedral-sim/src/conversation.rs':'pub mod checkpoint;\n',
    'crates/cathedral-sim/src/attention.rs':'pub mod checkpoint;\n',
    'crates/cathedral-sim/src/checkpoint/mod.rs':'pub mod social;\n',
}
comparisons = []
for name, addition in ordinary.items():
    before = git('show','HEAD:' + name)
    after = (root / name).read_bytes()
    assert after.count(addition.encode()) == 1, name
    reduced = after.replace(addition.encode(),b'',1)
    comments_only = name.endswith('/attention.rs')
    if comments_only:
        strip_docs = lambda b: b'\n'.join(line for line in b.split(b'\n') if not line.lstrip().startswith(b'///'))
        assert strip_docs(reduced) == strip_docs(before), name
    else:
        assert reduced == before, name
    comparisons.append({'path':name,'baseline_sha256':sha(before),'current_sha256':sha(after),'removed_module_bytes':addition,'remaining_bytes_identical':not comments_only,'remaining_non_doc_bytes_identical':True,'additional_doc_comments':comments_only})
fixtures = {}
...
report = {'checked_at_utc':stamp,'result':'passed','source_scope':SOURCE_SCOPE,'source_manifest_sha256':sha((e/'source_hashes.json').read_bytes()),'source_files_checked':len(manifest),'changed_paths_all_covered':True,'changed_paths':sorted(changed),'historical_fixtures_and_docs_unchanged':fixtures,'module_wiring_comparisons':comparisons}
...
print(json.dumps({'result':'passed','source_files':len(manifest),'changed_paths':len(changed),'scoped_format_checks':len(checks),'historical_fixture_files':len(fixtures)}))
