...
assert len(final) == len(before) + 3
assert len(publication['added_paths']) == 3
...
for name in ('owner-command-audit.json', 'release-build-audit.json',
             'release-smoke-audit.json', 'release-performance-audit.json',
             'fixture-audit.json'):
    report = json.loads((OUT / name).read_bytes())
    assert report['source_manifest_sha256'] == sha(LEG / 'source_hashes.json')
    if name == 'owner-command-audit.json':
        assert report['all_originals_and_archives_verified']
    else:
...
original = Path('/tmp/alibi-m2c-final-staged-diff-check.log')
