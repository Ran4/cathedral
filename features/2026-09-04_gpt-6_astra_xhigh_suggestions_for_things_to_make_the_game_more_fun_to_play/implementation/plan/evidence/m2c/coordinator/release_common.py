...
REMOVED_BUILD_ENVIRONMENT = (
    'LDFLAGS', 'RUSTFLAGS', 'CARGO_ENCODED_RUSTFLAGS', 'CARGO_BUILD_RUSTFLAGS',
    'RUSTC_WRAPPER', 'RUSTC_WORKSPACE_WRAPPER',
)
REMOVED_PROBE_ENVIRONMENT = (
    'ALIBI_CONTINUATION_MODE', 'ALIBI_CONTINUATION_SAMPLES', 'ALIBI_CONTINUATION_REPORT',
    'ALIBI_CONTINUATION_FIXTURE', 'ALIBI_CONTINUATION_READ_FIXTURE',
    'ALIBI_HYDRATION_MODE', 'ALIBI_HYDRATION_SAMPLES', 'ALIBI_HYDRATION_REPORT',
    'ALIBI_HYDRATION_FIXTURE', 'ALIBI_HYDRATION_INITIAL',
    'ALIBI_HYDRATION_EXPECT_INCOMPATIBLE', 'ALIBI_COMPLETE_WORLD_ID',
)
...
def capture(label, command, destination, extra_environment=None, binary=None):
    ...
    original = Path(f'/tmp/alibi-m2c-{label}.stdout-stderr.log')
    timing = Path(f'/tmp/alibi-m2c-{label}.time')
    ...
    for key in REMOVED_BUILD_ENVIRONMENT + REMOVED_PROBE_ENVIRONMENT:
    ...
    start = {
        ...
        'inherited_build_environment': {
            key: os.environ.get(key) for key in (
                ...
                'CARGO_TARGET_DIR', 'CARGO_BUILD_RUSTFLAGS', 'RUSTC_WRAPPER', 'RUSTC_WORKSPACE_WRAPPER',
            )
        },
        'removed_build_environment': list(REMOVED_BUILD_ENVIRONMENT),
        'removed_probe_environment': {key: os.environ.get(key) for key in REMOVED_PROBE_ENVIRONMENT},
