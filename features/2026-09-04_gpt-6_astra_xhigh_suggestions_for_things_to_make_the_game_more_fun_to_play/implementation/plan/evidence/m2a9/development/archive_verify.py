...
base=Path('features/2026-09-04_gpt-6_astra_xhigh_suggestions_for_things_to_make_the_game_more_fun_to_play/implementation/plan/evidence/m2a9')
...
current={str(p):sha(p.read_bytes()) for p in sorted(set(paths))}
assert current==manifest,'source/assets scope changed after final commands'
freeze=json.loads((base/'source_freeze.json').read_text())
assert sha((base/'source_hashes.json').read_bytes())==freeze['manifest_sha256']
final={}
for name in ['checkpoint_final','public_final','layout','workspace_path_retry']:
...
for mode in ['authored','populated']:
    ...
    smokes[mode]={'file':str(copy),'sha256':sha(copy.read_bytes()),'samples':2,'claim':'functional debug smoke only, preceding final source-backed queue validation and test/whitespace additions; coordinator release measures final frozen source'}
...
result={'status':'Owner verification passed; coordinator review/release acceptance pending','source_freeze':freeze,'source_files_verified':len(manifest),'commands_verified':len(commands),'final_verification':final,'workspace':{'command_name':'workspace_path_retry',**final['workspace_path_retry']},'smokes':smokes,'format':'Eight new owner/probe files scoped formatted; independent public test also scoped formatted. Ordinary source matches HEAD after module wiring removal; existing clock/ledger checkpoint prefixes are byte-identical with narrow formatted helpers appended.','development_failures':[r['name'] for r in commands if r['exit_code']!=0],'lossless_logs':'Every command stdout+stderr captured before summaries; deterministic gzip mtime=0, original/archive/decompressed SHA-256 verified. Every command has a source manifest from START; final focused/layout/workspace share one unchanged manifest. The failed workspace_serial bytes are retained exactly as captured; disk exhaustion interrupted its linker diagnostic, so the archive cannot recover output that never reached the original file.'}
