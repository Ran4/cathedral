...
## M5 final verification — 2026-09-06

No free parameter, salience band or affinity moved. The final band, full-day ON/OFF cost and
allocation records are in `../m5_measurements.md`, with raw data under `../m5_evidence/`. Bed reaches
eight wards at 2.01 hours and 446 carriers at the last headless sample; off-affinity Craft stays in
one ward at all 48 samples, with 0.132 expected crossings at 23.56 hours and 0.798516 nightfall
confinement. The unchanged test harness still covers 24.06 hours / 0.136 expected crossings. The
flat/deleted-salience streams are byte-identical across 625 lines. Same-trade reporting remains six
wards / 128 carriers and 12 warm same-trade exits per day (four per witness), matching M2's correction.

The full-day 20,000-person pair observes +6.18% user CPU and +16 MiB peak RSS, within the 15% /
32 MiB bounds. This is one coarse-step ON/OFF pair, not a replacement for M2's repeated game-pump
measurements. The unchanged six-hour ignored crowd guard passed with 14,027,250 store bytes over
100,745 holdings; the full-day headless store peaks at 16,078.8 KiB.

A further accounting correction supersedes the earlier footprint figures without rewriting their
history: allocated vector capacity, sealed-source strings, frozen household/craft data, distinct
receipt mouths and hearsay memo are now included. BTree node overhead remains an estimate. The
synthetic 20,519-body bound at six carried rows, 256 facts and saturated receipt sets is 20,123,494
bytes; adding 5,746,199 bytes for the conservative all-resident consequence/projection cache bound
gives **25,869,693 bytes < 33,554,432 (32 MiB)**. Actual cache peaks are reported separately from
PollenCensus.store_bytes. The canary measures a 16,993-byte saturated prompt (three bullets,
2,181 added bytes) and the unchanged 137,179-byte snapshot.
