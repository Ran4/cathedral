...
## M3 — reproducible re-measurement setup (2026-09-05)

Committed harness: `crates/cathedral-backends/tests/knowledge_provider_probe.rs`, explicit ignored
`prepare_m3_provider_sheets`; provider runner: `fire_probes.py` beside this record. The setup below
was fixed **before** firing either provider; there is no seed search or response selection.

The scenario personas do resolve by exact name into the authored cast. `SEEDS.json` under
`sheets/m3_garble/` records their real ids, the complete one-row packs, views, and rendered lines.
Every pack is the first installed row of a fresh world: **sequence 0**, the allocator's actual first
value. World day is 2, the arrest's day is 0; hops are the original scenario's. The full authored
roster provides the stable garble vocabulary, the real area map supplies adjacency, and only the
true subject is added to the reader's `knows`, as the original scenario explicitly requires.
Unknown substitutions still render as roles through the actual `knowledge::render_line`.

This uses option (a) in the earlier deferral: the original `v6_both_turnj2` full sheets are preserved
byte-for-byte except the knowledge bullet, which is rendered by the shipped Rust implementation.
The arrest template now has real `{place}`/`{day}` slots; its fixed Stone House destination remains
fixed. The low-band fixture's rejected possessive `{subject}'s mill` becomes `the mill of {subject}`
so it passes the production loader. The corrected M0b Osanne own line is retained. Non-witness Q4
mouths cite the specified earlier mouth: hops 1 Osanne, hops 2 Ansel, hops 3 Gile, hops 4 Havise.
These are explicit fixture provenance assignments, not a claim that a simulation run produced
these exact chains. The band pair has subject garbling only, matching its real placeholders.

Twelve sheets per provider: Q4's eight plus both bands at hops 3 and 4. OpenAI first, then moonshot;
one call per sheet, no automatic retries. Models are explicitly `gpt-5.6-luna` / `kimi-k3`.
Score by the unchanged own-clause rule extended for noticeable subject/place/day differences,
with the exact stop-word list and all 28 lexical comparisons from the earlier re-measurement
contract. Raw replies, stderr, input hashes, and lexical metrics are retained below `replies/m3_*`.

## M3 — Q4 re-measurement verdict (2026-09-05): PASS on both providers

All **24 calls succeeded on their first attempt**. Inputs: `sheets/m3_garble/`; raw outputs and
call records: `replies/m3_openai/` and `replies/m3_moonshot/`. `METRICS.json` contains every spoken
reply, all 28 pairs and the MD5s. No shipped prose changed and no alternate seed was tried.

| Provider | Distinct / 8 | Identical pairs | Jaccard mean / max / pairs ≥ 0.60 |
|---|---:|---:|---|
| openai / gpt-5.6-luna | **8 / 8** | 0 | 0.295 / 0.667 / 1 |
| moonshot / kimi-k3 | **8 / 8** | 0 | 0.152 / 0.409 / 0 |

The scoring is slot-based where there is no own clause, exactly as the predeclared M3 extension
allows. 01 repeats the yesterday version; 02 corrects the place to the Dawn Bearer; 03 is the own
witness; 04 gives the butcher / Slate Cistern version plus a referral; 05 gives the three-days-past
version; 06 gives the cargo worker / Slate Cistern version; 07 corrects to the Seraph / yesterday;
08 is the plain witness. These are eight distinct accounts under both providers, rather than
crediting a provenance paraphrase as personality. OpenAI 04/05 also supply referrals and 07
explicitly contradicts the asked location. Threshold 2 therefore clears its **≥ 6/8 on OpenAI**
condition against the prior 3/8 baseline for this fixed seed. One sample per persona is evidence
for this probe, not a claim that every possible seed/provider completion will pass.

**Failures retained in the raw record:** moonshot 05 answers “Aye, it's true. They took him…”
against a question naming Grigor, although its sheet names an unnamed animal worker. Its changed
day still counts for distinctness, but it loses the subject correction and overstates hearsay.
Moonshot 02 assigns a gender to the unnamed chandler; 04/06 say they do not know Grigor even though
the fixture knows him. OpenAI 07 repeats the map label's awkward “at Next to…” syntax. The live
rendering also exposes doubled prepositions in some area labels (`at At Slate Cistern`); recorded
as a prose-quality limitation, not silently repaired in the frozen template during measurement.

The erosion pair was re-fired too: both low-band hops-3 and hops-4 replies remain explicitly
hearsay and redirect the original named ask toward the unnamed garbled subject's trade. Thus
`know_hedge_low_hops3` now has its required second observation on both providers. The top pair is
**partial** on both: they repeat the alleged visit as common knowledge but retain “I only know” /
“what I've heard” framing instead of consistently speaking flatly. No new rung or probability is
introduced to chase that variability. `known_from` was exercised at every non-witness Q4 remove;
unknown sources stayed roles, and Petronel could name the Havise she actually knows.

## M4 — C1 topic probe setup (2026-09-06)

The ignored `prepare_m4_provider_sheets` in `knowledge_provider_probe.rs` renders the original
with/no-occasion Q5 scenarios through the shipped prompt renderer, replacing only the verb
example with the plan's unrelated **bread** example. The true occasion is the player's allegation
about Odo taking a cut from salt carts (**law**). Two sheets per provider, one first attempt each,
OpenAI `gpt-5.6-luna` and moonshot `kimi-k3`; raw replies and input hashes are retained in
`replies/m4_*`. No response selection or automatic retries. An emitted `raise_word` must classify
the allegation, rather than imitate the bread example. Under-firing is recorded and accepted by
C1; the no-occasion case must not emit the verb.

`SETUP.json` records the real cast ids and hearing setup. The with-occasion sheet is armed through
production `note_assertion`, and both sheets assert the conditional fence matches
`may_raise_word`. The original no-occasion history is a **render-only no-stamp fixture**: its
24-byte-plus ordinary statement would itself pass the generic assertion prefilter if dispatched
as speech. Thus this pair measures the model's response to presence/absence of the offered verb;
the simulation's occasion gating is covered separately by integration tests. Original scenario
prose and all earlier evidence remain unchanged.

## M4 — C1 verdict (2026-09-06): UNDER-FIRE on both providers

All four first-attempt calls succeeded. Both with-occasion replies use only `say`, explicitly
profess ignorance, and direct the player to the Tallage's workers. Both no-occasion replies answer
the sacks conversation in speech. **Zero `raise_word` calls, zero bread-topic imitations, zero
unoffered calls.** Raw completions and per-input SHA-256/call records are under `replies/m4_openai/`
and `replies/m4_moonshot/`; rendered sheets are under `sheets/m4_topic/`, and the two copied
scenario files under `scenarios/q5b_*` differ from Q5 only in the example line.

This is the explicitly accepted C1 under-fire outcome. It cannot establish classification accuracy
because neither model chose the verb. The shipped example remains unchanged; no prompt revision
or selective re-fire follows this result. Simulation and fake-backend tests establish that an
eligible claim can be raised and traced back to the player; these live samples show that the two
providers may still choose ordinary cautious speech instead.

## M5 — close-out (2026-09-06)

M1 shipped the measured `v6_both` text byte for byte: its 24 string keys and 22-line ignorance
paragraph, unconditional immediately before “Use ONLY the verbs listed below”, at the position
M0b tested. M3’s Q4 remeasurement with actual garbling is recorded above (both providers 8/8
distinct under the stated clause/slot rule, with the misread subject and partial hedge erosion
retained). M4’s C1 verdict is also above: both providers under-fired on the offered occasion,
accepted under the existing rule; neither invented a new raise on the no-occasion fixture. The
M4 conditional bread fence is the separately authorized interaction repair. No further prompt
prose change, scoring round or provider call is owed by M5; the frozen prompts remain the record.
