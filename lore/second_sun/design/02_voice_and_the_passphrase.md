...
Under `fake_backend: true` every flow above runs offline, identically every time: the matcher is already deterministic; Ashe's performance comes from the scripted fake cast (fixed lines per verdict); the speech line (§9) and drive-mode injection (§11) carry input; pass state is seeded (`pass_name: "Sef"`, sign "Belwyn's, unlit", next sign fixed, `pass_history: ["Cobb", "Ib"]` per design/04, plus one never-cell-paid registry burial, `Ansel` — Ansel is the FAIL-wrong name for rung-1 tests, Cobb the FAIL-stale name for rung-3 tests); Vell's rung-4 decision follows a fixed rule. Test matrix, one integration test each in the Python suite (`uv run --offline --no-project python -m unittest discover -s tests`):
