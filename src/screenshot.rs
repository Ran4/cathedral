...
struct ScreenshotNamer {
    last_stamp: String,
    next_index: u32,
    pending: Option<requests::Receipt>,
}
...
fn capture_screenshot_on_key(
    ...
    capture: requests::CaptureContext,
) {
    if let Some(receipt) = &namer.pending {
        match receipt.outcome() {
            requests::Outcome::Pending | requests::Outcome::Encoding => {}
            requests::Outcome::Saved => {
    ...
    match capture.submit(&mut commands, path) {
        Ok(receipt) => namer.pending = Some(receipt),
        Err(reason) => warn!("Screenshot refused: {reason:?}"),
    }
}
